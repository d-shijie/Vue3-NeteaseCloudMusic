    ...
    <section>
      <ul>
        <li :style="{background: (index % 2) === 0 ? '#2b2b2b' : '#2f2f2f'}" @click="gotoDetail(item.id)"
          class="item cursor-pointer flex items-center p-10px hover:bg-#373737" v-for="(item, index) in djlists"
          :key="index">
          ...
          <span class="text-#696969 text-13px w-100px flex items-center ">
            <svg-icon name="play_outline"></svg-icon> {{ formatCount(item.playCount) }}
          </span>
          ...
          <span class="text-#696969 text-13px w-200px">
            by <span :style="{color: searchKeyword(i, String(route.query.keywords)) ? '#85b9e6' : ''}"
              v-for="(i, index1) in item.dj.nickname" :key="index1">
              {{ i }}
            </span>
          </span>
        </li>
...
const params = reactive({
  ...
  type: 1009 as SearchType,
  offset: 1,
  limit: 20
})
const djlists = ref<any[]>([])
...
const getDjlists = () => {
  searchApi({
    ...
    total.value = res.data.result.djRadiosCount
    djlists.value = res.data.result.djRadios
  })
}
getDjlists()
